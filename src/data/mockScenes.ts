import type { SceneCard, SceneData } from '../types/che';

export const mockSceneCards: SceneCard[] = [
  {
    id: 'scene-fitness-tonight',
    sceneType: 'fitness',
    title: '一起健身',
    timeHint: '20:10 左右',
    timeLabel: '20:10 左右',
    timePrecision: 'approximate',
    description: '晚点热身，先晒一下太阳。',
    status: 'scheduled',
    linkedPlanId: 'workout',
    conversationMode: 'scene',
    sortOrder: 1,
  },
  {
    id: 'scene-meal-evening',
    sceneType: 'meal',
    title: '一起吃饭',
    timeHint: '晚些时候',
    timeLabel: '晚些时候',
    timePrecision: 'period',
    description: '他在想今晚做点清淡的，也提醒你别空腹硬扛。',
    status: 'availableNow',
    linkedPlanId: 'plan-001',
    conversationMode: 'scene',
    sortOrder: 2,
  },
  {
    id: 'scene-watch-late',
    sceneType: 'watch',
    title: '一起看电影',
    timeHint: '睡前',
    timeLabel: '睡前',
    timePrecision: 'period',
    description: '选一部轻一点的电影吧，今晚可以慢一点。',
    status: 'flexible',
    linkedPlanId: 'plan-003',
    conversationMode: 'scene',
    sortOrder: 3,
  },
  {
    id: 'scene-study-now',
    sceneType: 'study',
    title: '一起专注',
    timeHint: '现在也可以',
    timeLabel: '现在也可以',
    timePrecision: 'open',
    description: '他在桌边收尾稿子，你也可以安静做自己的事。',
    status: 'availableNow',
    linkedPlanId: null,
    conversationMode: 'scene',
    sortOrder: 4,
  },
];

export const mockScenes: SceneData[] = [
  {
    id: 'study',
    title: '一起专注',
    shortTitle: '专注',
    conversationMode: 'scene',
    entryLabel: '一起专注一会儿',
    setting: '桌面很干净，窗边有一点下午的光。',
    cheStatusHint: '他把耳机摘下一边，示意你可以开始。',
    starterMessage: '我这会儿在处理一点工作。你也把自己的事放在旁边，我们先各自专心一会儿。',
    allowedQuickReplies: ['开始专注', '休息五分钟', '换个任务'],
    isDeepEntry: false,
    mood: 'mist',
    textTone: 'light',
    focalPoint: '58% 36%',
  },
  {
    id: 'meal',
    title: '一起吃饭',
    shortTitle: '吃饭',
    conversationMode: 'scene',
    entryLabel: '一起吃点热的',
    setting: '厨房里有一点热气，锅里的汤还在小火滚着。',
    cheStatusHint: '他在等你把饭收拾好。',
    starterMessage: '先好好吃饭。今天不用一边吃一边绷着。',
    allowedQuickReplies: ['看看吃什么', '简单一点', '晚点再吃'],
    isDeepEntry: false,
    mood: 'warm',
    textTone: 'light',
    focalPoint: '50% 38%',
  },
  {
    id: 'fitness',
    title: '一起健身',
    shortTitle: '健身',
    conversationMode: 'scene',
    entryLabel: '一起动一动',
    setting: '健身包放在门边，运动鞋已经换好。',
    cheStatusHint: '他刚换好运动鞋，准备出门。',
    starterMessage: '别一下子太狠。先动起来就算数。',
    allowedQuickReplies: ['开始热身', '轻量一点', '练完报备'],
    isDeepEntry: false,
    mood: 'green',
    textTone: 'light',
    focalPoint: '54% 32%',
  },
  {
    id: 'watch',
    title: '一起看电影',
    shortTitle: '看电影',
    conversationMode: 'scene',
    entryLabel: '陪我看一会儿',
    setting: '客厅灯开得很低，茶几上放着遥控器。',
    cheStatusHint: '他去倒了杯水，坐回沙发边。',
    starterMessage: '今晚就轻一点吧。你想吐槽剧情也可以。',
    allowedQuickReplies: ['开始看', '换一部', '先聊两句'],
    isDeepEntry: false,
    mood: 'mist',
    textTone: 'light',
    focalPoint: '52% 42%',
  },
  {
    id: 'sleep',
    title: '睡前聊会儿',
    shortTitle: '睡前',
    conversationMode: 'scene',
    entryLabel: '睡前慢慢说',
    setting: '房间里只留了一盏小灯，外面安静下来。',
    cheStatusHint: '他把声音放轻了一点。',
    starterMessage: '今天辛苦了。你可以少说一点，我也会在。',
    allowedQuickReplies: ['说两句', '安静一会儿', '准备睡了'],
    isDeepEntry: false,
    mood: 'night',
    textTone: 'light',
    focalPoint: 'center',
  },
  {
    id: 'idle',
    title: '随便待一会儿',
    shortTitle: '日常',
    conversationMode: 'scene',
    entryLabel: '无事闲聊',
    setting: '客厅很安静，窗外有一点晚风。',
    cheStatusHint: '他刚合上电脑，靠在沙发边。',
    starterMessage: '来了？今天想安静待一会儿，还是随便说点什么。',
    allowedQuickReplies: ['安静待会儿', '随便聊聊', '说今天'],
    isDeepEntry: false,
    mood: 'mist',
    textTone: 'light',
    focalPoint: '56% 34%',
  },
  {
    id: 'deep_room',
    title: 'Deep Room',
    shortTitle: '安静聊聊',
    conversationMode: 'deep',
    entryLabel: '安静聊聊',
    setting: '夜晚的客厅只留了一盏暖灯，窗边沙发安静下来。',
    cheStatusHint: '他坐近一点，但没有催你开口。',
    starterMessage: '你不用一下子讲清楚。先说最压着你的那一块就好。',
    allowedQuickReplies: ['慢慢说', '先坐一会儿', '从一件事说起'],
    isDeepEntry: true,
    mood: 'night',
    textTone: 'light',
    focalPoint: '50% 40%',
  },
];
