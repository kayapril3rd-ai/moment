...
export function getPlanComputedStatus(plan: Pick<UserPlan, 'startTime' | 'timeLabel' | 'durationMinutes' | 'status'>, dateKey: string, now = new Date()): ComputedEventStatus {
...
export function getStatusLabel(status: ComputedEventStatus, plan: Pick<UserPlan, 'status' | 'inviteStatus'>): string {
  ...
  if (plan.inviteStatus === 'accepted') return '已约好';
  return '邀请澈';
}
